 ...
 * @date 2021/12/21
 */
public interface MiniweiboUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new MiniweiboUser(user.getUserId(), user.getDeptId(), user.getUsername(),
	...
	/**
	 * 通过用户实体查询
	 * @param miniweiboUser user
	 * @return
	 */
	default UserDetails loadUserByUser(MiniweiboUser miniweiboUser) {
		return this.loadUserByUsername(miniweiboUser.getUsername());
	}
